...
const OrganHubPage: React.FC = () => {
  ...
  const id: OrganId | null = ORGAN_IDS.includes((organId ?? '') as OrganId)
    ? (organId as OrganId)
    : null;
                      ...
                      {detail!.factors.map((f) => (
                        <li key={String(f.labelKey)} className="text-sm text-[#4A5A55]">
                          <span className="inline-block min-w-28">
                            {String(f.labelKey).startsWith('advanced.lab.')
                              ? t(String(f.labelKey) as TKey)
                              : String(f.labelKey).startsWith('universe.')
                                ? t(String(f.labelKey) as TKey)
                                : String(f.labelKey)}
                          </span>
                          <span className="font-extrabold text-[#0F4C3A]">{f.value}</span>
