// Validates scripts/tunisia-100-proposal.json against:
//  1) schema (5 langs, mealType, region, cal_100 range)
//  2) internal duplicate Arabic names
//  3) duplicate Arabic names already in live Supabase DB
//  4) duplicate Arabic names already in the local 200-name Tunisian set
...
const proposal = JSON.parse(fs.readFileSync(path.join(__dirname, 'tunisia-100-proposal.json'), 'utf8'));
const existing = JSON.parse(fs.readFileSync(path.join(__dirname, '_db-names.json'), 'utf8'));

if (!fs.existsSync(path.join(__dirname, '../src/data/tunisian-full-100-USDA.json'))) {
  console.error('missing source file');
  process.exit(1);
}
const local = JSON.parse(fs.readFileSync(path.join(__dirname, '../src/data/tunisian-full-100-USDA.json'), 'utf8'));
...
const kitchens = new Set(existing.length ? ['existing'] : []);

console.log('===== TUNISIA 100 PROPOSAL VALIDATION =====');
...
console.log(`dups vs local 200  : ${localDups.length}${localDups.length ? ' -> ' + localDups.join(' | ') : ''}`);
...
process.exitCode = schemaErrors.length || internalDups.length ? 1 : 0;