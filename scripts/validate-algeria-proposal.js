// Validates scripts/algeria-100-proposal.json against:
...
//  4) duplicate Arabic names already in the local 200-name Algerian set
...
const proposal = JSON.parse(fs.readFileSync(path.join(__dirname, 'algeria-100-proposal.json'), 'utf8'));
const existing = JSON.parse(fs.readFileSync(path.join(__dirname, '_db-names.json'), 'utf8'));

const local = JSON.parse(fs.readFileSync(path.join(__dirname, '../src/data/algerian-full-100-USDA.json'), 'utf8'));
...
console.log('===== ALGERIA 100 PROPOSAL VALIDATION =====');
