...
const JSON_PATH = path.join(__dirname, 'src', 'data', 'saudi-full-100-USDA.json');
...
const allDishes = data.categories.flatMap((cat) => cat.dishes);
...
const dishRow = (d) => {
  ...
  if (d.region_confidence) row.region_confidence = d.region_confidence;
  return row;
};
