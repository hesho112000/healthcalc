...
const prop = JSON.parse(fs.readFileSync(path.join(__dirname, 'scripts/syria-200-proposal.json'), 'utf8'));
...
for (let i = 0; i < TOTAL; i++) {
  ...
  try {
    ...
    if (existing) {
      ...
      console.log(`Skip ${i + 1}/${TOTAL} [syrian] "${nameAr}" already exists (id=${existing.id})`);
      continue;
    }
    const { error } = await supabase.from('dishes').insert({
      ...
      source: 'levant-syria-2026 - Verified against Syrian Culinary Heritage',
      confidence: null,
    ...
    inserted++;
    console.log(`Insert ${i + 1}/${TOTAL} [syrian] "${nameAr}" (region=${d.region}, meal=${d.mealType}, cal=${d.cal_100})`);
  } catch (err) {
    failed++;
